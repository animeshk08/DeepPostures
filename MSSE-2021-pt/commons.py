...
def window_generator_subject(win_size_10s, data):
    for x_seq, _, y_seq in data:
        x_window = []
        y_window = []
        for x, y in zip(x_seq, y_seq):
            x_window.append(x)
...
def get_dataloaders(
    ...
    if test_subjects_data:
        test_data = IterDataset(
            window_generator_subject(bi_lstm_win_size, test_subjects_data)
        )
